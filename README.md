...
Agents hit friction constantly — dead-end tool calls, broken links, missing helpers, footgun configs — and silently push through without telling anyone. The signal evaporates. `papercut` gives an agent a one-line way to file the complaint at the moment it happens, and gives you (or another agent) a way to review the backlog and fix the actual problems in your repo, your tooling, your docs.
...
```bash
papercut add "text"            # file a papercut (also: papercut log, or pipe stdin to add -)
papercut list                  # open papercuts, severity-first then newest, JSON envelope
papercut list --format md      # human review digest
papercut resolve pc_9f2c41d0   # mark one fixed (full or unique-prefix id)
papercut schema                # full machine contract — agents self-orient with this
papercut doctor                # validate the log file
```
...
annoyances, major for time sinks, blocker for hard walls. Run `papercut schema`
once if you need the full contract.
```

Then periodically: `papercut list --format md` and fix what your agents keep tripping over.
...
Everything an agent needs is in `papercut schema`: commands and flags with read-only/appends annotations, env vars, record shapes, error codes, and the exit-code dictionary (0 success · 2 usage · 65 bad input · 66 not found · 70 internal · 74 I/O · 75 lock timeout, retryable · 77 permission denied · 78 config). Empty results are exit 0, never errors.
